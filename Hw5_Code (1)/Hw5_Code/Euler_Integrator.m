...
for ii = 2:T_steps
    % grab previous time and state position
    t_prev = time(ii-1);
    X_prev = X(ii-1, :);
    
    % Calculate the derivative at the previous step using given function
    X_dot = f(t_prev, X_prev.');
    
    % apply Euler and update X to: X_prev + transpose(X_dot) * DeltaT
    % transpose X_dot back to a row vector to store it in the X matrix.
    X(ii, :) = X_prev + (X_dot.') * DeltaT;
