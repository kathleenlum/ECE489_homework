...
q0 = [pi/3; pi/3];  %Initial angle
q_dot0 = [1; -1];   %Initial angular velocity
T = 6;             %Simulation time
deltaT = 10/1000;
%% ------------------------------------------------------------------------
...
X0 = [q0; q_dot0;];
tic
[time, X] = ode45(@Planar_Manipulator,[0 T],X0);

%[time, X] = Euler_Integrator(@Planar_Manipulator,[0 T],X0, deltaT);
toc
