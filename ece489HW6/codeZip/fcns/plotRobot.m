...
if isempty(h)
    ...
    h{1} = plot3(chain(1,:),chain(2,:),chain(3,:),...
        'ok','LineWidth',4); % Robot stick figure
    ...
    h{7} = DrawFrame(eye(4), 0.3); % Origin frame, frame 0
    h{8} = DrawFrame(T04, 0.1); % End effector frame, frame 4
    hold off
else
    ...
    h{6} = DrawCube([10; 1.5; 0.5]*0.0254,[5; 0; 0]*0.0254,...
        RobotColor,T03, h{6});
    h{7} = DrawFrame(eye(4), 0.3, h{7});
    h{8} = DrawFrame(T04, 0.1, h{8});
end
